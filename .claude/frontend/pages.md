...
16 routes. All under `src/pages/`. Routing in `src/App.tsx`.
...
| `/research` | `Research.tsx` (lazy) | `POST /v1/research/ask`, `GET /v1/research/queries`, `POST /v1/research/queries/{id}/{approve,dismiss}`, `DELETE /v1/research/queries/{id}` | Single-turn ask box → Claude verdict + evidence + (sometimes) ProposedActionCard. History list with status filter. Optional `tickers` input (comma-sep) + `force_skip_context_gate` for the TV-context gating layer. When `status='needs_context'`, renders `ContextNeededBanner` with per-ticker "Attach for X" deeplinks → `/tv-context/<TICKER>`. Laptop-only — Railway shows banner pointing back at the toggle. |
