...
3. Fires `schedule.runner.request_wake()` — historically nudged a 429-deferred daily run. Under the queue this is no longer needed (worker auto-pulls the next item) but the call is harmless and kept for the rare external-caller case. See [schedule.md](schedule.md) and [tech_debt.md](../status/tech_debt.md).
