...
Phase 1 of the trust-sprint roadmap. Builds per-(ticker, horizon, model) error metrics over evaluated predictions, surfaces drift, posts to Telegram. See [.claude/roadmap.md](../status/roadmap.md) for the broader plan.
...
Drift alerts and the daily digest both use `app/notifications/telegram.py`. The notifier no-ops gracefully when `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` are unset (logged once at startup). Setup steps in [backlog.md](../status/backlog.md) "Unlock #1".
