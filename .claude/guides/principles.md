...
8. **Trust the model before trusting the trader.** Phase 1 (accuracy + drift) gates Phase 3 (opportunities). We'd rather know Kronos is wrong than build features that assume it's right. See [roadmap-shipped.md](../status/roadmap-shipped.md) for the sequencing.
...
| Postgres-only durability | Throughput at >10× scale | [tech_debt.md](../status/tech_debt.md) "Tier 2 queue (Redis + arq)" |
| In-process workers | Multi-machine resilience (laptop dies → manual restart) | [tech_debt.md](../status/tech_debt.md) Tier-2 path |
...
| Pragmatic testing (mostly backend, 269 tests) | UI bugs only caught at preview/manual | [backlog.md](../status/backlog.md) — would add Playwright E2E only when scale forces |
...
| Belt-and-braces | Code surface bloats over time | Monthly review of [tech_debt.md](../status/tech_debt.md) |
| Telegram only | No email, no Slack, no push API redundancy | [roadmap-shipped.md](../status/roadmap-shipped.md) Phase 4 — multi-channel deferred until Telegram proves valuable |
| Tailscale-only laptop | Public-internet laptop access requires a different tunnel | [backlog.md](../status/backlog.md) "Public tunnel for laptop" |
| Hardcoded opportunity rules | Tunable strategies require redeploy | [opportunities.md](../modules/opportunities.md) — DSL deferred until thresholds proven |
...
- [tech_debt.md](../status/tech_debt.md) — code cruft we deliberately left, with eviction triggers.
- [backlog.md](../status/backlog.md) — features deferred for product reasons.
