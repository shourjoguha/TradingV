...
In-process FIFO drain for analysis-job submissions. Replaces the previous 429-on-busy + 5-min-poll-recovery pattern. See [.claude/tech_debt.md](tech_debt.md) for deferred decisions (concurrency slot gate kept as belt-and-braces; Tier-2 Redis-arq reserved for when Kronos parallelism matters).
...
`app/schedule/runner.py::_tick` and `_fallback_tick` both call `queue.enqueue(source='schedule'|'fallback')` instead of `analysis.submit_run`. The 429-deferred path (`pending_run`, `retry_minutes`, `last_run_status='deferred_429'`) is unreachable under the queue — see [tech_debt.md](tech_debt.md) for the column-cleanup trigger.
